...
public class Bartok : MonoBehaviour
{
    ...
    public void CardClicked(CardBartok tCB)
    {
        ...
        // Act differently based on whether it was a card in the hand, or a card in the drawPile
        switch (tCB.state)
        {
    ...
    void LayoutGame()
    {
        ...
        foreach (BSlotDef tSD in layout.bSlotDefs)
        {
            pl = new Player();
            pl.handBSlotDef = tSD;
            players.Add(pl);
        ...
        for (int i = 0; i < numStartingCards; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                // ^ By setting the timeStart before calling addCard, we override the automatic
                //  setting of timeStart in CardBartok.MoveTo()
                // Add the card to the player's hand
                // The module (%4) results in a number 0 to 3
    ...
    public void PassTurn(int num = -1)
    {
        ...
        Vector3 lPos = CURRENT_PLAYER.handBSlotDef.pos + Vector3.back * 5;
