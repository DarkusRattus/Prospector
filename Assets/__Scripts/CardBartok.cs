...
public class CardBartok : Card {
    ...
    public void MoveTo(Vector3 ePos, Quaternion eRot)
    {
        ...
        // Otherwise, it starts at timeStart.
        // This way, if timeStart is already set, it won't be overwritten
        if(timeStart == 0)
        {
            timeStart = Time.time;
        }
	...
	void Update () {
        switch (state)
        {
            // All the to____ states are ones where the card is interpolating
            case CBState.toHand:
            ...
            case CBState.to:
                ...
                else if (u >= 1) // If u >= 1, we're finished moving
                {
                    ...
                    {
                        // ... then use SendMessage to call CBCallback method with this as the parameter
                        reportFinishTo.SendMessage("CBCallback", this);
                        // After calling SendMessage(), reportFinishTo must be set to null so that the card
                        //  doesn't continue to report to the same GameObject every subsequent time it moves 
                        reportFinishTo = null;
                    }
